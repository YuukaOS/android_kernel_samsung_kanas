...
/** This is reintroduced, so it is not needed
		...
		const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		...
		j_dbs_info->prev_cpu_idle_up = get_cpu_idle_time(j, 0, 0);
...
static void dbs_check_cpu(int cpu)
{
	...
	/* Check for frequency increase */
	total_idle_ticks = get_cpu_idle_time(cpu, 0, 0);
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_dbs_info->prev_cpu_idle_up = get_cpu_idle_time(cpu, 0, 0);
